...
# Basic input format
# gen_sub -set setfile -x jobtype -in input -ref refname -nd ND -nc NC -nt NT -wt WT -- 
...
EOF
    ...
    # -nc and -nd
        ...
        ND=`echo "scale=0; ${ND}" | bc`
        NCPUNODE=`echo "scale=0;${NC}/${ND}" | bc`
...
EOF
    ...
    TPROC=`echo "scale=0;${NCPU}/${TRED}*${ND}" | bc`
}
...
function print_HOWTO_ {
    ...
    Available flags
        ...
        -set -x  -in  -ref  -nd  -nc  -nt  -wt  -name  -help
    ...
    Typically an alias is defined beginning with 'X', which does not require -set flag. For example:
        ...
        Xcrys17 -name mgo_band -nc 32 -nd 2 -x Pcrys -in mgo.d12 -ref no -wt 02:00 -x Pprop -in mgo.d3 -ref mgo -wt 00:30
    ...
    is 2 hours and for 'mgo.d3' is 30 minutes. 2 nodes / 32 cores in
    total are used.
