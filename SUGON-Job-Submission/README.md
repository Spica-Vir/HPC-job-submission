...
A general job submission script for parallel programs for the SLURM batch system on national HPC service SCNet SUGON. Based on Linux Bash Shell.
...
| `BUDGET_CODE`             | \[Not Used\]             | Grant code for HPC service                                               |
| `QOS`                     | \[Not Used\]             | Acceptable Quality of service. By default the code reads the first entry |
| `PARTITION`               | xhhctdnormal xhacnormalb | Acceptable job partition. By default the code reads the first entry      |
